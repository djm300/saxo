...
if __name__ == "__main__":
    ...
    # Only start a new authorization if no valid token is available
    if not client.tokens or client._is_expired():
        logger.info("No valid token available; starting authorization flow.")
    ...
    logger.info("Access token: %s...", client.tokens.get("access_token", "")[:20])
