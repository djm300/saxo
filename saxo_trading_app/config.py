...
class Config:
    def __init__(self):
        ...
        if self.SIMULATION_MODE:
            self.AUTH_ENDPOINT = _load_config_value("SAXO_AUTH_ENDPOINT", "https://sim.logonvalidation.net/authorize", json_config=params_config)
            self.TOKEN_ENDPOINT = _load_config_value("SAXO_TOKEN_ENDPOINT", "https://sim.logonvalidation.net/token", json_config=params_config)
            self.TOKEN_FILE = "saxo_tokens_sim.json"
            self.CLIENT_ID = _load_config_value("SAXO_CLIENT_ID", "89da08eeb25c428a9099f768cdb1696e", json_config=params_config)
            self.BASE_URL = "https://gateway.saxobank.com/sim/openapi"
            logger.info("Running in SIMULATION mode.")
        else:
            self.AUTH_ENDPOINT = _load_config_value("SAXO_AUTH_ENDPOINT", "https://live.logonvalidation.net/authorize", json_config=params_config)
            self.TOKEN_ENDPOINT = _load_config_value("SAXO_TOKEN_ENDPOINT", "https://live.logonvalidation.net/token", json_config=params_config)
            self.TOKEN_FILE = "saxo_tokens_live.json"
            self.CLIENT_ID = _load_config_value("SAXO_CLIENT_ID", "28d17c462242447f94c4b0767c41a552", json_config=params_config)
